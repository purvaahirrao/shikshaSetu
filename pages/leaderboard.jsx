      ...
      <div className="px-5 pt-6 pb-8 space-y-6">
        {rows.length === 0 && (
          <p className="text-center text-slate-500 text-sm py-8">
            No student accounts on this device yet. Register as a student and practice — your XP will show here.
          </p>
        )}
        ...
        {rows.length > 0 && (
          <div className="card p-0 overflow-hidden animate-fade-up shadow-card" style={{ animationDelay: '100ms' }}>
            {(rows.length >= 3 ? rest : rows).map((r, i) => (
              <div
                key={`${r.id}-${i}`}
                className={`flex items-center gap-3 p-4 border-b border-slate-50 last:border-0 transition-colors ${
                  r.isUser ? 'bg-brand-50' : 'bg-white hover:bg-slate-50'
                }`}
              >
                <span
                  className={`font-display font-900 w-6 text-center text-lg ${
                    r.isUser ? 'text-brand-500' : 'text-slate-400'
                  }`}
                >
                ...
                <div className="flex-1">
                  <p
                    className={`font-display font-800 text-sm leading-tight ${
                      r.isUser ? 'text-brand-800' : 'text-slate-800'
                    }`}
                  >
                    {r.name}
                    {r.isUser && <span className="text-brand-500 text-xs font-700 ml-1">(you)</span>}
                  </p>
        ...
        {rows.length > 0 && (
          <p className="text-[11px] text-slate-400 text-center">
            Ranks use XP from activity on this device (solve, scan, quiz, chat).
          </p>
        )}
      </div>
    </AppShell>
