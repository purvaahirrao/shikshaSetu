...
export default function ProfilePage() {
  ...
  const handleChangePassword = () => {
    ...
    try {
      const db = JSON.parse(localStorage.getItem('ss_registered_users') || '{}');
      const key = user.email?.toLowerCase();
      if (!key || !db[key]) { setPwError('Account not found.'); return; }
      if (db[key].password !== oldPw) { setPwError('Current password is incorrect.'); return; }
  ...
  const statsConfig =
    role === 'teacher' ? [
      ...
      { label: 'Child quiz avg', value: extraStats.parentChildAcc != null ? `${extraStats.parentChildAcc}%` : '—', icon: Star, color: 'text-amber-500', bg: 'bg-amber-100' },
      { label: 'Child activity', value: extraStats.parentChildActs, icon: BarChart3, color: 'text-orange-500', bg: 'bg-orange-100' },
    ] : [
      { label: 'Day Streak', value: st.streak, icon: Zap, color: 'text-amber-500', bg: 'bg-amber-100' },
      { label: 'Total XP', value: st.xp.toLocaleString(), icon: Star, color: 'text-purple-500', bg: 'bg-purple-100' },
    ];
      ...
      <div className="px-5 py-6 space-y-6 animate-fade-in">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex flex-wrap gap-2 mt-3 justify-center">
            {role === 'student' && (
              <>
                <span className="px-3 py-1 bg-brand-50  text-brand-700  text-xs font-700 rounded-full">Class {cls || '?'}</span>
            ...
            {role === 'teacher' && (
              <>
                <span className="px-3 py-1 bg-indigo-50 text-indigo-700 text-xs font-700 rounded-full">{subject || 'Teacher'}</span>
                <span className="px-3 py-1 bg-purple-50 text-purple-700 text-xs font-700 rounded-full">{exp || 'Experienced'}</span>
              </>
            )}
            {role === 'parent' && (
              <>
                <span className="px-3 py-1 bg-amber-50  text-amber-700  text-xs font-700 rounded-full">Child: {childName || 'N/A'}</span>
                <span className="px-3 py-1 bg-orange-50 text-orange-700 text-xs font-700 rounded-full">Class {childClass || '?'}</span>
              </>
            )}
          </div>
              ...
              <>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {LANGUAGES.map(l => (
                      <button key={l.value} onClick={() => setLang(l.value)} className={`flex-1 py-2 rounded-xl text-sm font-600 transition-all ${lang === l.value ? 'bg-brand-500 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>{l.label}</button>
                    ))}
                  </div>
                </div>
                <div>
            ...
            {role === 'teacher' && (
              <>
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SUBJECTS.map(s => (
                      <button key={s} onClick={() => setSubject(s)} className={`px-3 py-1.5 rounded-xl text-sm font-600 transition-all ${subject === s ? 'bg-indigo-500 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>{s}</button>
                    ))}
                  </div>
                </div>
                <div>
