...
import { formatRelativeTimeI18n } from '../../../services/timeI18n';
...
export default function StudentDetailPage() {
  ...
  if (!decoded || !student) {
    return (
      <AppShell title={t('page_student')} back onBack={() => router.back()}>
        <div className="flex flex-col items-center justify-center min-h-[60vh] px-5">
          <p className="text-slate-500 text-lg">{t('student_not_found')}</p>
  ...
  const subjectBars = subjectRows.slice(0, 6).map((r) => ({
    key: r.name,
    label: translateSubjectDisplayName(r.name, t),
    pct: r.pct,
  ...
  return (
    <AppShell
      title={student.name || t('page_student')}
      back
      onBack={() => router.back()}
    >
      <div className="px-5 pt-5 pb-4 space-y-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-indigo-200 text-sm mt-0.5">
                {t('home_class')} {student.class || '?'}
              </p>
              <div className="flex gap-2 mt-2">
                <span className="text-xs bg-white/20 text-white px-2 py-0.5 rounded-lg font-700">
                  {t('lb_xp', { n: Math.round(p.xp || 0) })}
                </span>
                <span className="text-xs bg-white/20 text-white px-2 py-0.5 rounded-lg font-700">
                  {t('student_streak_line', { n: p.streak || 0 })}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3 animate-fade-up" style={{ animationDelay: '60ms' }}>
          {[
            ...
            {
              k: 'sol',
              label: t('student_solved'),
              value: p.questionsSolved || 0,
              icon: BookOpen,
        ...
        <div className="space-y-3 animate-fade-up" style={{ animationDelay: '120ms' }}>
          <h3 className="font-display font-800 text-slate-700 text-base">{t('student_subject_mix')}</h3>
          <div className="card space-y-4">
            {subjectBars.length === 0 && (
              <p className="text-sm text-slate-400">{t('student_no_subject_yet')}</p>
            )}
              ...
              <div key={key}>
                <div className="flex justify-between mb-1.5">
                  ...
                  <p className={`text-sm font-800 ${pct >= 30 ? 'text-emerald-500' : 'text-slate-500'}`}>
                    {t('student_q_share', { q: questions, pct })}
                  </p>
                </div>
        ...
        {weak.length > 0 && (
          <div className="space-y-3 animate-fade-up" style={{ animationDelay: '160ms' }}>
            <h3 className="font-display font-800 text-slate-700 text-base">{t('student_focus_areas')}</h3>
            <div className="flex flex-wrap gap-2">
              {weak.map((area) => (
                ...
                >
                  <AlertTriangle size={12} /> {t('student_less_practice')}{' '}
                  {translateSubjectDisplayName(area, t)}
                </span>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-3 animate-fade-up" style={{ animationDelay: '200ms' }}>
          <h3 className="font-display font-800 text-slate-700 text-base">{t('student_recent_activity')}</h3>
          <div className="card p-0 overflow-hidden">
            {recent.length === 0 && (
              <p className="p-4 text-sm text-slate-400">{t('student_no_recent')}</p>
            )}
            {recent.map((r, i) => (
              <div
                key={`${r.time}-${i}`}
              ...
              >
                ...
                <span className="text-[11px] font-600 text-slate-400 shrink-0">
                  {formatRelativeTimeI18n(r.time, t)}
                </span>
              </div>
            ))}
          </div>
